...
pub const RULES: &[RtkRule] = &[
    ...
    RtkRule {
        pattern: r"^(pnpm|npm|yarn)\s+(run|exec|test|build|lint|install|add|remove)\b|^(npx\s+|pnpm\s+)?nest\s+(build|start|test)\b",
        rtk_cmd: "rtk npm",
        rewrite_prefixes: &["npx nest", "pnpm nest", "nest", "yarn", "pnpm", "npm"],
        category: "PackageManager",
        savings_pct: 70.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^(\./mvnw|mvnw|mvn)\s+(compile|package|clean|install|test|verify|generate-sources|versions:\S+)\b",
        rtk_cmd: "rtk mvn",
    ...
    RtkRule {
        pattern: r"^(\./gradlew|gradlew|gradle)\s+(build|test|clean|check|assemble|spotlessCheck|spotlessApply|jacocoTestReport)\b",
        rtk_cmd: "rtk gradle",
